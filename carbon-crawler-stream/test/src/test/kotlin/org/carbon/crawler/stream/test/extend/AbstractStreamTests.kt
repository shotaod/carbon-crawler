...
abstract class AbstractStreamTests : InitializingBean {
    ...
    private lateinit var appRegistryOperations: AppRegistryOperations
    ...
    override fun afterPropertiesSet() {
        try {
            dataFlowOperations = DataFlowTemplate(URI(property.serverUri))
    ...
    protected fun deployStream(stream: StreamDefinition) {
        ...
        val streamProperties = HashMap<String, String>()
        streamProperties["app.*.logging.file"] = "\${PID}-test.log"
        streamProperties["app.*.endpoints.logfile.sensitive"] = "false"
        // Specific to Boot 2.x applications, also allows access without authentication
        streamProperties["app.*.management.endpoints.web.exposure.include"] = "*"

        streamProperties.putAll(stream.deploymentProperties)
     ...
     * @return the available StreamDefinition REST resource.
     */
    protected fun rollbackStream(stream: StreamDefinition): StreamDefinitionResource? {
     ...
     * @return the available StreamDefinition REST resource.
     */
    private fun rollbackStream(stream: StreamDefinition, streamVersion: Int): StreamDefinitionResource? {
    ...
    private fun registerApps() {
        val resource = property.streamRegistrationResource
     ...
     * @return the available StreamDefinition REST resource.
     */
    protected fun streamAvailable(stream: StreamDefinition): StreamDefinitionResource? {
        ...
        while (!streamStarted && attempt < property.deployPauseRetries.toInt()) {
            val streamIter = streamOperations.list().content.iterator()
            while (streamIter.hasNext()) {
                resource = streamIter.next()
                if (resource!!.name == stream.name) {
                    status = resource.status
                    logger.info("Checking: Stream=" + stream.name +
                        ", status = " + status +
                        ", status description = " + resource.statusDescription)
                    if (status == "deployed") {
                        val urlsAvailable = platformHelper.setUrlsForStream(stream)
                        if (urlsAvailable) {
                            streamStarted = true
                        }
                    }
        ...
        if (streamStarted) {
            ...
            logger.info(String.format("Stream '%s' NOT started with status: %s.  Description = %s",
                stream.name, status, statusDescription))

            throw IllegalStateException("Unable to start stream " + stream.name +
                ".  Definition = " + stream.dsl)
        }
        return resource
    }
     ...
     * attribute.
     */
    private fun deploymentPause() {
        try {
            Thread.sleep((property.deployPauseTime.toInt() * 1000).toLong())
        } catch (ex: Exception) {
            // ignore
        }
    ...
    private fun getLogFileUrl(url: String): String {
        ...
        val actuatorUrl = String.format("%s/actuator", url)
        var responseEntity: ResponseEntity<String>? = null
        try {
            responseEntity = restTemplate.exchange(actuatorUrl, HttpMethod.GET, null, String::class.java)
        } catch (e: Exception) {

        }

        return if (responseEntity != null && responseEntity.statusCode == HttpStatus.OK) {
            String.format("%s/actuator/logfile", url)
        } else {
            String.format("%s/logfile", url)
        }
    }
    ...
    protected fun waitForLogEntry(app: Application, vararg entries: String): Boolean {
        ...
        if (exists) {
            logger.info("Matched all '" + StringUtils.arrayToCommaDelimitedString(entries)
                + "' in logfile for instance " + instance + " of app " + app.definition)
        } else {
            logger.error("ERROR: Couldn't find '" + StringUtils.arrayToCommaDelimitedString(entries)
                + "' in logfiles for " + app.definition + ". Dumping log files.\n\n")
            for ((_, value) in logData) {
                logger.error("<logFile> =================")
                logger.error("Log File for $value\n$value")
                logger.error("</logFile> ================\n")
            }
        }
        return exists
    ...
    protected fun dataFlowOperations(): DataFlowOperations {
        return dataFlowOperations
    }

    enum class PlatformTypes(val value: String) {
        LOCAL("local"),
